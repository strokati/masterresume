...
export async function POST(req: NextRequest) {
	...
	if (!['professional', 'confident', 'warm'].includes(tone)) {
		return new Response('Invalid tone. Must be: professional, confident, or warm.', { status: 400 });
	}
	...
	try {
		const result = await generateCoverLetter(userId, applicationId, tone as 'professional' | 'confident' | 'warm', providerId);
