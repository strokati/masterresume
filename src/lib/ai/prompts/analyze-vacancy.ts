...
interface ResumeSummaryData {
	...
	workCompanies?: { companyName: string; roles: { jobTitle: string; startDate?: string | null; endDate?: string | null }[] }[];
...
export function summarizeMasterResume(data: ResumeSummaryData): string {
	...
	if (data.workCompanies?.length) {
		const jobs = data.workCompanies.map((c) =>
			c.roles.map((r) => `${r.jobTitle} at ${c.companyName} (${r.startDate ?? ''} - ${r.endDate ?? 'present'})`).join('; '),
		);
		parts.push(`Work Experience: ${jobs.join(', ')}`);
	}

	if (data.educations?.length) {
		parts.push(`Education: ${data.educations.map((e) => `${e.degree} in ${e.field} from ${e.institution}`).join(', ')}`);
	}
	...
	if (data.skills?.length) {
		...
		parts.push(`Skills: ${Object.entries(grouped).map(([cat, items]) => `${cat}: ${items.join(', ')}`).join('; ')}`);
	}

	if (data.certifications?.length) {
		parts.push(`Certifications: ${data.certifications.map((c) => `${c.name} (${c.issuer})`).join(', ')}`);
	}

	if (data.projects?.length) {
		parts.push(`Projects: ${data.projects.map((p) => `${p.name}${p.technologies ? ` [${p.technologies}]` : ''}`).join(', ')}`);
	}

	if (data.volunteeringRoles?.length) {
		parts.push(`Volunteering: ${data.volunteeringRoles.map((v) => `${v.role} at ${v.organization}`).join(', ')}`);
	}

	if (data.publications?.length) {
		parts.push(`Publications: ${data.publications.map((p) => `${p.title}${p.publisher ? ` (${p.publisher})` : ''}`).join(', ')}`);
	}
