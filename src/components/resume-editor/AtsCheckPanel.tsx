		...
		<Card>
			<CardContent className="p-5 space-y-4">
				...
				{displayResult && !isLoading && (
					<AtsCheckContent result={displayResult} copiedKeyword={copiedKeyword} onCopied={setCopiedKeyword} />
				)}
			</CardContent>
		</Card>
	);
...
function AtsCheckContent({
	...
	const scoreColor = result.overallScore >= 70 ? 'text-green-600' : result.overallScore >= 40 ? 'text-yellow-600' : 'text-red-600';
	const scoreRing = result.overallScore >= 70 ? 'stroke-green-500' : result.overallScore >= 40 ? 'stroke-yellow-500' : 'stroke-red-500';
						...
						<div className="flex-1">
							...
							<div className="flex flex-wrap gap-1">
								{result.keywordCoverage.found?.map((kw, i) => (
									<Badge key={i} variant="outline" className="text-[0.65rem] border-green-200 text-green-700 dark:border-green-800 dark:text-green-300">
										{kw}
									</Badge>
								))}
							</div>
...
function PriorityDot({ priority }: { priority: string }) {
	const color = priority === 'HIGH' ? 'bg-red-500' : priority === 'MED' ? 'bg-yellow-500' : 'bg-green-500';

	return (
		<span className={`mt-1.5 h-2 w-2 rounded-full shrink-0 ${color}`} />
	);
}
