	...
	return (
		<Card>
			<CardContent className="p-5 space-y-4">
				...
				{displayAnalysis && !isLoading && (
					<AnalysisResult analysis={displayAnalysis} onCopied={setCopiedKeyword} copiedKeyword={copiedKeyword} />
				)}
			</CardContent>
		</Card>
	);
				...
				<div>
					...
					<div className="flex flex-wrap gap-1.5">
						{analysis.mustHaves.map((item, i) => (
							<Badge key={i} variant="outline" className="text-[0.7rem] border-red-200 text-red-700 dark:border-red-800 dark:text-red-300">
								{item}
							</Badge>
						))}
					</div>
				...
				<div>
					...
					<div className="flex flex-wrap gap-1.5">
						{analysis.niceToHaves.map((item, i) => (
							<Badge key={i} variant="outline" className="text-[0.7rem] border-yellow-200 text-yellow-700 dark:border-yellow-800 dark:text-yellow-300">
								{item}
							</Badge>
						))}
					</div>
