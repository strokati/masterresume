...
export function WorkProjectCard({ project, roleId }: { project: WorkProject; roleId: string }) {
	...
	return (
		<>
			<Card className="border-dashed">
				...
				{!collapsed && (
					<CardContent className="px-3 pb-3 pt-0 space-y-1 text-sm">
						...
						{project.contribution && <p className="text-muted-foreground"><span className="font-medium">Contribution:</span> {project.contribution}</p>}
						{project.outcome && <p className="text-muted-foreground"><span className="font-medium">Outcome:</span> {project.outcome}</p>}
						{techs.length > 0 && (
			...
			<WorkProjectDialog open={editOpen} onOpenChange={setEditOpen} roleId={roleId} project={project} />
		</>
	);
}
