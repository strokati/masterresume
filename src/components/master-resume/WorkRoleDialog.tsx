...
function WorkRoleDialogForm({
	...
	const form = useForm<{ title: string; startDate?: string; endDate?: string }>({
		resolver: zodResolver(CreateWorkRoleSchema.pick({ title: true, startDate: true, endDate: true })),
		values: { title: role?.title ?? '', startDate: role?.startDate ?? '', endDate: role?.endDate ?? '' },
	});
	...
	return (
		<form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
			...
			<StringListEditor label="Responsibilities" items={responsibilities} onChange={setResponsibilities} />
				...
				<div className="flex flex-wrap gap-1">
					{technologies.map((tag) => (
						<Badge key={tag} variant="secondary" className="gap-1">
							{tag}
							<button type="button" onClick={() => setTechnologies(technologies.filter((t) => t !== tag))}>
								<X className="h-3 w-3" />
							</button>
						</Badge>
					))}
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
				...
				{open && <WorkRoleDialogForm key={role?.id ?? 'new'} companyId={companyId} role={role} onOpenChange={onOpenChange} />}
			</DialogContent>
		</Dialog>
	);
}
