...
import { createSkill, updateSkill, deleteSkill, reorderSkills } from '@/server/actions/master-resume';
...
function SkillCategory({
	...
	function handleDragEnd(event: DragEndEvent) {
		...
		startTransition(async () => {
			try {
				await reorderSkills(resumeId, reordered.map((s) => s.id));
			} catch {
	...
	function addSkill() {
		...
		startTransition(async () => {
			try {
				await createSkill(resumeId, { name, category: category === 'Other' ? undefined : category });
				setNewSkill('');
			} catch {
