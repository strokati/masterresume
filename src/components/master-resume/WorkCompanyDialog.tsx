...
function WorkCompanyDialogForm({
	...
	const form = useForm<CreateWorkCompanyInput>({
		...
		defaultValues: {
			...
			employmentType: (company?.employmentType as CreateWorkCompanyInput['employmentType']) ?? undefined,
			...
			<div className="grid gap-4 sm:grid-cols-2">
				...
				<div className="space-y-1.5">
					...
					<Select
						...
						onValueChange={(v) => {
							setEmploymentType(v ?? '');
							setValue('employmentType', v === '' ? undefined : v as CreateWorkCompanyInput['employmentType']);
						}}
					>
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent>
				...
				{open && <WorkCompanyDialogForm key={company?.id ?? 'new'} resumeId={resumeId} company={company} onOpenChange={onOpenChange} />}
			</DialogContent>
		</Dialog>
	);
}
