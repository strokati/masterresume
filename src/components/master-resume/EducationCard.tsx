...
export function EducationCard({
	...
	const dateRange = [education.startDate, education.endDate ?? 'Present'].filter(Boolean).join(' – ');
	...
	return (
		<>
			<Card>
				<CardHeader className="flex flex-row items-center justify-between space-y-0 p-3">
					<div className="flex items-center gap-2 min-w-0">
						{dragHandleProps && (
							<span {...dragHandleProps} className="cursor-grab touch-none text-muted-foreground hover:text-foreground">
					...
					<div className="flex items-center gap-1 shrink-0">
						{education.honors && (
							<Badge variant="secondary" className="text-xs mr-1">{education.honors}</Badge>
						)}
			...
			<EducationDialog open={editOpen} onOpenChange={setEditOpen} resumeId={resumeId} education={education} />
		</>
	);
}
