		...
		<Dialog open={open} onOpenChange={handleOpenChange}>
			<DialogContent className="sm:max-w-lg">
				<DialogHeader>
					...
					<DialogDescription>
						{step === 'upload' && 'Upload an existing resume (PDF or DOCX) and AI will extract it into your master resume.'}
				...
				{step === 'processing' && (
					<div className="space-y-4">
						...
						{processingDone && !importError && (
							<Button className="w-full" onClick={() => { handleResultReady(); setStep('review'); }}>
								Review Results
							</Button>
						)}
						{processingDone && importError && (
							<Button variant="outline" className="w-full" onClick={() => { setStep('upload'); resetState(); }}>
								Try Again
							</Button>
						)}
					</div>
		...
		<div className="space-y-4">
			<div className="grid grid-cols-2 gap-2">
				{Object.entries(counts).map(([section, count]) => (
					<div key={section} className="flex items-center justify-between rounded-lg bg-muted/50 px-3 py-2 text-sm">
