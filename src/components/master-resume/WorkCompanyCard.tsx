...
export function WorkCompanyCard({
	...
	return (
		<>
				...
				<CardHeader className="flex flex-row items-center justify-between space-y-0 p-3">
					<div className="flex items-center gap-2 min-w-0">
						...
						{company.employmentType && (
							<Badge variant="outline" className="text-xs shrink-0">{company.employmentType}</Badge>
						)}
			...
			<WorkCompanyDialog open={editOpen} onOpenChange={setEditOpen} resumeId={resumeId} company={company} />
			<WorkRoleDialog open={roleDialogOpen} onOpenChange={setRoleDialogOpen} companyId={company.id} />
		</>
	);
}
