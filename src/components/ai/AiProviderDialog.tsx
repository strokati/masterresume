...
export function AiProviderDialog({
	...
	const form = useForm<Omit<UpsertAiProviderInput, 'providerId'>>({
		...
		defaultValues: {
			...
			baseUrl: existingConfig?.baseUrl ?? (providerId === 'ollama' ? 'http://localhost:11434/v1' : ''),
			displayName: '',
		},
	});
	...
	function handleTest() {
		...
		const overrides = (apiKeyValue || model) ? {
			...
			<DialogContent className="sm:max-w-lg">
				...
				<form onSubmit={handleSubmit(onSubmit)} className="space-y-4" key={formKey}>
					<div className="space-y-1.5">
						<Label htmlFor="apiKey">API Key {hasExistingKey ? '(leave empty to keep current)' : '*'}</Label>
						<Input
							id="apiKey"
							type="password"
					...
					{needsBaseUrl && (
						<div className="space-y-1.5">
							...
							<Input
								id="baseUrl"
								placeholder={providerId === 'ollama' ? 'http://localhost:11434/v1' : 'https://api.example.com/v1'}
								{...register('baseUrl')}
							/>
						</div>
					)}
					...
					{testResult && (
						<div className={`text-sm rounded-xl p-3 ${testResult.success ? 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-300' : 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-300'}`}>
